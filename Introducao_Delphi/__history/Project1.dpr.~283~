...
{$APPTYPE CONSOLE} // fala que o programa vai ser rodado no console (compiler directive)

{$R *.res} // incluiu um arquivo externo de recursos, o "*" fala que o nome desse arquivo será o mesmo no projeto (Project1.exe)

uses // bibliotecas utilizadas (unidades):
...
var // declara as variáveis (são case insensitive)
  ...
  Balance: Double; // variável Double

  MyArray: IntArray; // varíavel do tipo que eu criei (IntArray)
  DArray: DynArr; // mesma coisa, porém com o outro tipo (DynArr)
  MultArray: array [0..3, 0..2] of Integer; // Array multidimensional

  W, X, Y, Z: Integer; // declara varais variaveis de mesmo tipo
...
begin
  try
    ...
    // ARRAYS:
    MyArray[1] := 0; // atribui a primeira posicao do Myarray para ser igual a 0
    Writeln('Array 0: ', MyArray[1].ToString);
    setlength(DArray, 5); // delimita que o array dinamico terá 5 espaços

    // LOOPING:
    for W := 1 to 10 do // define um loop de iteravel W (inicializado no inicio do codigo) que começa com 1 e vai até 10
      MyArray[W] := W; // para cada iteracao de W, o mesmo valor a cada loop é atribuido a posicao da lista

    for W := 1 to 5 do
      Writeln('MyArray: ', MyArray[W]);

    // LOOPING DE ARRAY MULTIDIMENSIONAL:
    for W := 1 to 3 do
        for X:= 1 to 2 do
           MultArray[W,X]:= W+X;
    ...
    // LOOPING COM WHILE:
    W := 1;
    while W <= 20 do // While de números ímpares até 15
      begin // sempre que tiver varias opcoes em um loop, devo utilizar o begin e o end, se nao, o codigo so executa a primeira linha
        if (W mod 2) = 0 then // se o resto da divisao de W / 2 for 0, o código só continua para o próximo loop e não chega no final do while
          begin
            W := W + 1;
            continue;  // pula pra cima (while W <= 20 do) para continuar o loop
          end;
        if W = 15 then  // quando W = 15, o While Loop é cancelado
          break;  // nao precisa de begin e end porque é só 1 linha de comando, break quebra o loop
        Writeln('W: ', W); // final do While
        W := W + 1;
      end;
